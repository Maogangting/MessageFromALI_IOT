...
namespace Aliyun.MNS.Sample
{
    ...
    public class SyncOperationSample
    {
        ...
        private const string _queueName = "aliyun-iot-a1xvHPI3zVC";
        ...
        static void Main(string[] args)
        {
            ...
            {
#if true
                // 批量消费消息 ， 当前一次最多16个message
                /* 14. Batch Receive message */
                BatchReceiveMessageResponse batchResponse = null;
                try
                {
                    var nativeQueue = client.GetNativeQueue(_queueName);
                    batchResponse = nativeQueue.BatchReceiveMessage(batchSize, 3);
                    Console.WriteLine("Batch receive message successfully, status code: {0}, messages count {1}",
                        batchResponse.HttpStatusCode, batchResponse.Messages.Count);
                    Console.WriteLine("----------------------------------------------------");
                    foreach (var message in batchResponse.Messages)
                    {
                        // 针对每个message的处理
                        /*
...
*/
                        ...
                        byte[] outputb = Convert.FromBase64String(message_body);
                        message_body = System.Text.Encoding.Default.GetString(outputb);
                        Console.WriteLine("MessageBody: {0}", message_body);

                        // Json
                        //string jsonText = "{\"zone\":\"海淀\",\"zone_en\":\"haidian\"}";
                        JObject jo = (JObject)JsonConvert.DeserializeObject(message_body);
                        string payload = jo["payload"].ToString();
                        // Json 嵌套
                        ...
                        */
                        ...
                        //base64 to  string
                        outputb = Convert.FromBase64String(payload);
                        payload = System.Text.Encoding.Default.GetString(outputb);
                ...
                if (batchResponse != null && batchResponse.Messages.Count > 0)
                {
                    try
                    {
                        var nativeQueue = client.GetNativeQueue(_queueName);
            ...
            {
                var nativeQueue = client.GetNativeQueue(_queueName);
                nativeQueue = client.GetNativeQueue(_queueName);
...
// 20181106 BUG1: MNS里面的message的先后排序混乱，如何设置先进先出。